...
def build_skills_manifest(*, include_write: bool = True) -> str:
    ...
    out.append("")
    out.append("**IMPORTANT — wrap every tool invocation with markers** so the")
    out.append("UI can show the user which tool you used. Before the command,")
    out.append("print a line `[[dlw_tool name=<tool_name> input=<json>]]`;")
    out.append("after the command (when you have the result), print a line")
    out.append("`[[dlw_tool_result name=<tool_name> ok=<true|false>]]`.")
    out.append("Example for a search:")
    out.append("")
    out.append("```")
    out.append("[[dlw_tool name=search_huggingface_models input={\"query\":\"deepseek\"}]]")
    out.append("curl -s \"https://huggingface.co/api/models?search=deepseek&...\"")
    out.append("[[dlw_tool_result name=search_huggingface_models ok=true]]")
    out.append("```")
    out.append("")
    out.append("These markers are stripped before the user sees the reply;")
    out.append("they ONLY drive the decision-chain UI. Missing markers ≠ error,")
    out.append("but the UI will not be able to show the tool was used.")
    out.append("")
