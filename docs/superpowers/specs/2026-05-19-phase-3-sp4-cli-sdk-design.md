...
- **In-process e2e (no network, no new dep):** SDK/CLI tests drive the real ASGI app via `httpx.ASGITransport(app=create_app())`, authenticating with a real system-JWT minted by the existing api-test helper (`tests/conftest.make_app_with_state` / `principal_headers`). This exercises the true SDK→FastAPI→service path, catching contract drift the v2.0 doc's `MockServer` would hide.
  - The sync `Client`/`AsyncClient` accept an optional injected `transport=` (httpx supports `ASGITransport` for both `httpx.Client` and `httpx.AsyncClient`) so tests point at the app without a live server. Production callers never pass `transport`.
