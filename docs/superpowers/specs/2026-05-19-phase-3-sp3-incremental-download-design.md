...
| Inherit `copy_object`/`os.link` fails (key gone, perms, ENOSPC) | subtask → `failed` → normal retry/re-plan (a later pass may download it); the premature ref/refcount from `diff_and_dedup` is reconciled by GC (refcount eventually correct) — acceptable |
