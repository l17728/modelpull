...
- [ ] **Step 1 (failing tests):** write `tests/api/test_auth_device.py` per spec §2. Use the existing app/client fixture + `principal_headers` (`tests/conftest.py:327`) to forge an approver. **Service-reject test wiring (reviewer-flagged):** `system_admin_token` defaults to `""` which DISABLES the service bypass (`principal.py:73`). To exercise the `is_service` path, set the env BEFORE the app is built: `monkeypatch.setenv("DLW_SYSTEM_ADMIN_TOKEN", "svc-tok")` + `get_settings.cache_clear()` before the `client` fixture builds the app (mirror the `_dev_mode` fixture pattern in `tests/api/test_auth_endpoints.py:28-38`), then send the header via the existing `service_headers(token)` helper (`tests/conftest.py:340`). Do NOT rely on mutating `app.state.settings` post-build. Tests:
...
- [ ] **Step 4 (POST /device):** add `@router.post("/device")` (no auth dep). Build `device_code = secrets.token_urlsafe(32)`; loop ≤5× generating `user_code` until insert succeeds (catch unique-violation / pre-check pending uniqueness). Insert `DeviceAuthSession(device_code_hash=_hash_code(device_code), user_code=uc, status="pending", expires_at=now+settings.device_code_ttl_seconds, interval_seconds=settings.device_poll_interval_seconds)`. **`write_audit` REQUIRES `resource_type` + `resource_id`** (see `services/audit.py` and the callback usage `auth.py:85-87`) — call `await write_audit(session, action="auth.device.authorize", resource_type="device_auth", resource_id=uc, outcome="success", tenant_id=None, actor_user_id=None)` best-effort. Commit. Return JSON {device_code, user_code, verification_uri=settings.device_verification_uri, verification_uri_complete=f"{...}?user_code={uc}", expires_in=settings.device_code_ttl_seconds, interval=settings.device_poll_interval_seconds}. (Use the `_session` dependency already in the file — it uses `get_engine()`, the same path the existing callback tests exercise.)
- [ ] **Step 5 (POST /device/approve):** `@router.post("/device/approve")`, `principal: Principal = Depends(require_principal)`, body `DeviceApproveReq`. If `principal.is_service`: raise HTTPException(403, {"code":"SERVICE_CANNOT_APPROVE"}). `select(DeviceAuthSession).where(user_code==_norm_user_code(body.user_code), status=="pending", expires_at>now).with_for_update()`. None → 404 {"code":"DEVICE_CODE_INVALID"}. If action=="deny": status="denied". Else: status="approved", user_id=principal.user_id, tenant_id=principal.tenant_id, role=principal.role, project_ids=list(principal.project_ids), approved_at=now. Audit (full signature) `await write_audit(session, action="auth.device.approve"|"auth.device.deny", resource_type="device_auth", resource_id=row.user_code, outcome="success", tenant_id=principal.tenant_id, actor_user_id=principal.user_id)`. Commit. Return {"status": status}.
- [ ] **Step 6 (POST /device/token):** `@router.post("/device/token")` (no auth), body `DeviceTokenReq`. `select(...).where(device_code_hash==_hash_code(body.device_code)).with_for_update()`. Helper to return `JSONResponse({"error": code}, status_code=400)`. None → invalid_grant. If `last_polled_at` and `(now-last_polled_at).total_seconds() < interval_seconds` → set last_polled_at=now, commit, slow_down. Set last_polled_at=now. If now>expires_at and status!="approved" (and !="consumed") → expired_token. Branch on status: pending→authorization_pending; denied→access_denied; consumed→expired_token; approved→ mint `issue_system_jwt(secret=settings.system_jwt_secret, user_id=row.user_id, tenant_id=row.tenant_id, role=row.role, project_ids=row.project_ids or [])`, set status="consumed", audit (full signature) `await write_audit(session, action="auth.device.token", resource_type="device_auth", resource_id=row.user_code, outcome="success", tenant_id=row.tenant_id, actor_user_id=row.user_id)`, commit, return 200 {access_token, token_type:"Bearer", expires_in:3600, tenant_id:row.tenant_id, role:row.role}. (Commit after the last_polled_at update even on the error branches so slow_down state persists. The `expires_in:3600` here is the minted JWT's TTL — `SYSTEM_JWT_TTL_SECONDS` — NOT the session `device_code_ttl_seconds`; keep them distinct.)
...
- [ ] **Step 1 (failing tests):** `tests/cli/test_login.py`. Read `tests/cli/test_cli_context.py` for the harness — it calls `cli.main([...])`. **CRITICAL invocation order (reviewer-flagged BLOCKER):** `-c/--config` is a TOP-LEVEL flag (`main.py:50`), parsed BEFORE the subcommand, so it MUST come first: `cli.main(["-c", str(tmp), "login", ...])` — NOT `login ... --config <tmp>` (that errors → exit 2). `--server`/`--token` are defined on the login subparser (mirroring `context set`) and go AFTER the subcommand. `login`/`logout` read `args.config` from the top-level flag (they do NOT define their own `--config`).
  - `test_login_device_writes_token`: monkeypatch `dlw.cli.handlers.device_authorize` → `{"device_code":"dc","user_code":"BCDF-GHJK","verification_uri":"/device","verification_uri_complete":"/device?user_code=BCDF-GHJK","expires_in":600,"interval":0}` and `dlw.cli.handlers.poll_for_token` → `{"access_token":"TOK","token_type":"Bearer","expires_in":3600,"tenant_id":1,"role":"tenant_operator"}`; run `cli.main(["-c", str(tmp), "login", "--no-browser", "--context", "test", "--server", "http://x"])`; assert exit 0 and `load_config(tmp)` has `auth.test.access_token == "TOK"` and `current_context == "test"`.
  - `test_login_token_shortcut`: monkeypatch `dlw.cli.handlers.device_authorize` to RAISE if called; run `cli.main(["-c", str(tmp), "login", "--token", "T", "--context", "test", "--server", "http://x"])`; assert token T written and device_authorize was never called.
  - `test_logout_clears_token`: pre-write a context with a token (`set_context("test", token="X", config_path=tmp)`), run `cli.main(["-c", str(tmp), "logout", "--context", "test"])`, assert token now unset.
- [ ] **Step 2: verify FAIL.**
- [ ] **Step 3 (parser):** in `cli/main.py` add `login` and `logout` subparsers. `login`: `--device-code` (store_true), `--no-browser` (store_true), `--context`, `--server`, `--token`, `--timeout` (float, default None → falls back to `expires_in`). `logout`: `--context`. **Do NOT add a `--config` to these subparsers** — `-c/--config` is the existing TOP-LEVEL flag (`main.py:50`) and `_login_cmd`/`_logout_cmd` read `args.config` from it. Define `--server`/`--token` ON THE login subparser (they shadow the top-level defaults of None; this is exactly how `context set` does it at `main.py:93-94` and is proven by its tests — pass them AFTER the subcommand).
