...
| W6-N | minor | Caught during T9 execution: plan suggested tests `await db_session.commit()` to make recovery's writes visible. But the `env` fixture (per-test) inserts Tenant id=1 / Project id=1 / Storage id=1 via flush+rollback for isolation. If the test commits, those rows persist; the next test's `env` fixture inserts duplicates → PK violation | Use `await db_session.flush()` instead of `commit()` in recovery tests. flush is sufficient because within the SAME transaction, subsequent `session.refresh()` or `select()` queries see the flushed UPDATEs. db_session rollback at test end cleans up |
