...
- [ ] **Step 1 (failing tests — write path):** in `tests/services/test_physical_keys.py` add per spec §2:
  ...
  - `test_record_ref_only_touches_physical_keys_for_sha`: seed a `StoragePhysicalKey` (`last_referenced_at=_old(...)`, sha=S) + a `StorageObject` (sha=S, refcount=1) + a `FileSubTask` row to satisfy the `subtask_object_refs` FK (`subtask_id` → `file_subtasks.id`); `await record_ref_only(session, tenant_id=, storage_id=, storage_key=, sha256=S, size=, subtask_id=<that subtask>)`; commit; reload the physical key → `last_referenced_at` advanced. **NOTE (reviewer-flagged):** `test_physical_keys.py` has NO FileSubTask/subtask seed helper — its bootstrap only seeds a `Tenant`. You MUST seed the FK chain yourself: `Tenant` → `DownloadTask` (FileSubTask has a FK to a task) → `FileSubTask` (read `src/dlw/db/models/` for `DownloadTask`/`FileSubTask` required columns — DownloadTask needs `repo_id`/`revision`/`path_template`, FileSubTask needs its task_id + path/index columns; mirror the FK-chain seeding used in `tests/api/test_executors.py` or `tests/services/test_local_reclaim.py`). Add the rows in parent-before-child `flush()` order to avoid executemany FK-order violations.
