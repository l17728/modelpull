...
| W5-J | important | Caught during M1 T1 execution: HF SDK's `GatedRepoError` is a SUBCLASS of `RepositoryNotFoundError`, so plan's except-clause order (`RepositoryNotFoundError` first, `GatedRepoError` second) makes the parent clause always fire — gated-repo errors mapped to `RepoNotFound` instead of `HfPrivateOrAuthRequired`. Two reviewers missed this Python class-hierarchy gotcha | Implementer swapped except order: `GatedRepoError` → `RepositoryNotFoundError` → `HfHubHTTPError` (most-specific to least). Now both reviewers' planned semantics work as intended |
