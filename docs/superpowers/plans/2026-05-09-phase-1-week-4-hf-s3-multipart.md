...
Expected: `INFO  [alembic.runtime.migration] Running upgrade <prev> -> <new>, add s3_key column to file_subtasks`. No errors.
...
@pytest.mark.slow
async def test_downloader_two_parts_exact_5mb(
    s3_bucket: str, monkeypatch: pytest.MonkeyPatch,
) -> None:
...
```python
@pytest.mark.slow
async def test_downloader_aborts_multipart_on_hf_error(
    s3_bucket: str, monkeypatch: pytest.MonkeyPatch,
) -> None:
    """If HF stream raises mid-flight, S3 multipart is aborted; no orphaned object."""
    def handler(request: httpx.Request) -> httpx.Response:
    ...
    # Verify no completed objects + no in-progress multipart uploads
    s3 = _boto3.client("s3", region_name="us-east-1")
...
def _is_transient_http(exc: BaseException) -> bool:
    """5xx HTTP + network/timeout = transient (retry-worthy).

    4xx errors (404 / 401 / 403) are NOT transient — config / repo issues
    won't fix themselves, so fail fast.
    """
...
- [ ] **Step 1: Register the `manual` pytest marker**

In `pyproject.toml`, find `[tool.pytest.ini_options].markers` (added in Week 1 for the `slow` marker). Add:
...
(Replace the existing `markers = [...]` list.)
...
- [ ] **Step 3: Verify the manual marker excludes the test from default runs**

```bash
uv run pytest --collect-only tests/e2e/ 2>&1 | grep -c smoke
```

Expected: 0 (the smoke test isn't collected because `manual` marker excludes it from default; only `-m manual` collects).
...
- [x] Backend pytest: 5 new \`hf_metadata\` tests + rewritten \`task_service\` tests (6) + 4 new \`api/tasks\` tests + 4 new \`scheduler\` tests + 9 \`downloader\` tests (skeleton + pipeline + abort + retry) + rewritten e2e — all green
- [x] Total tests: previous 73 + new ~28 = ~101 passing
- [x] Alembic migration up/down clean
- [x] \`pnpm typecheck/lint/test/build\` all green (no frontend changes)
- [x] Manual smoke (locally only, requires minio binary): \`pytest -m manual tests/e2e/test_hf_s3_smoke_local.py\` against \`sentence-transformers/all-MiniLM-L6-v2\` → all subtasks succeeded
