...
- **解决什么**：在分布式环境下保证 (a) 不双发 (b) 不丢数据 (c) 故障可恢复 (d) 状态机健全
- **涉及不变量**：5/6/7/9（fence + epoch）/ 26/32/34（multipart）/ 33（recovery 屏障）/ 35（WSS epoch）/ 42/43（lock + leader）— 共 14 条核心
- **必看一节**：§2 Fence Token / §3 三联校验 / §5 节点状态机；其他可作 reference
- **争议点**：α=1.0（13 §1.4）和 cooldown=60s（13 §4.3）在配置默认值表（13 §9.1.1）有统计学推导
