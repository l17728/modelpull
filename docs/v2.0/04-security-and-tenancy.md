...
- **解决什么**：modelpull 的安全模型 — OIDC 用户认证 + mTLS 执行器认证 + 多租户 RBAC + 配额 + 合规审计
- **涉及不变量**：1/2/3/4（凭证不离开/Executor 无长期凭证/Controller 不反连）/ 8（tenant_id）/ 15-19（AI）/ 30/31（本地凭证池）/ 36/37/41（AI sanitize / MCP 沙箱 / T2）/ 44/45（mTLS fingerprint / cn 数据出境）— 共 16 条
- **必看一节**：§1 租户模型 / §2 认证 / §3 凭证管理（**§3 头有 mental model 导览图**）/ §9 审计链
- **常见误解**：Tenant-级 HF Token vs Executor-本地 OOB token 是两套独立体系，详见 §3.1 + 14 §3 + 不变量 2 注解
