...
| 2026-05-07 | v2.0.16 | **PR 9（代码细节）** 修 10 项：CODE-09 OpenAPI 加 mTLS + HMAC body sig 鉴权 schemes；CODE-10 加 ProblemDetail (RFC 7807) + RateLimit headers (X-RateLimit-*) + Pagination headers；CODE-22 给 /executors/heartbeat 加 2 个完整 examples（healthyMidDownload / withCompletion）；CODE-11 executor-statefulset.yaml storageClassName fallback 链 + TGPS 与 graceful_shutdown 强关联；CODE-13 drain-executor.sh 单次 GET + jq // 0 防止 reassign 期间死等；CODE-14 gc-orphan-parts.sh 加 GNU coreutils 检查（macOS 友好错误）；CODE-18 ai-copilot-egress NetworkPolicy 修 podSelector（matchExpressions 同时覆盖 in-process MCP 和 sidecar）；CODE-17 CI 加 npm + pip cache（构建快 ~3min）；CODE-16 Grafana overview-dashboard 加 $datasource template variable（多 cluster 支持）；CODE-21 PR template 简化为 5 必勾 + details 折叠；DX-04 新增 question.yml issue template（无 required field 低门槛） |
| 2026-05-07 | v2.0.17 | **PR 10（最终收尾）** 修 ~12 项：CODE-20 Helm 加 3 个缺失模板（HPA / ServiceMonitor + PodMonitor / pre-install Migration Job）；OPS-V21-06 overview dashboard 加 8 个红绿灯总览面板（Controller role / PG lag / WSS conn % / Probe leader / 活跃告警数 / Recovery 状态 / Audit chain / DataIntegrity 24h）；OPS-V21-07 加 Loki dataLinks（panel 跳转关联日志）；OPS-V21-08 新增 RB-20 (DB pool exhausted) + RB-21 (TenantQuotaSpike) + RB-22 (Egress surge) 共 3 份 runbook；OPS-V21-08 加 3 条对应告警（DBPoolExhausted P0 / TenantQuotaSpike work-hours / EgressBytesSurge work-hours）；OPS-V21-10 AICostPerTenantSurge 改用相对 baseline（>3× 7d 均值 + 绝对下限 $0.5/h），告别"$5/day"对所有租户误用；DOC-12 02/03/04/05 章顶部加 3 行 TL;DR（解决什么 / 涉及不变量 / 必看一节） |
